...
description: Use when an implementation plan is written and approved and the work will be executed by a remote/headless Codex (Codex.ai, a cloud agent, or any session without this conversation's context) — typically to run unattended overnight. Hardens the plan into a self-contained contract and produces the kickoff prompt + push checklist. Do NOT use for plans executed in this same session.
---

# /overnight-implementation — hand a plan to a context-less Codex

## Overview

A plan written in *this* session leans on context that never reaches a remote
Codex: the repo's AGENTS.md, the superpowers skillset, the known-green test
...
then produces the exact kickoff prompt to paste into the remote Codex and the
...
your head or in AGENTS.md, it does not exist for the overnight worker — write it
into the plan.

## When to use

- A plan exists (via superpowers:writing-plans) and the human has approved it.
- Execution will happen in a **different, context-less session** — Codex.ai, a
...
|---|---|
| **Language** (British English everywhere; chat-with-human is the only other tongue) | A remote Codex defaults to US spelling and may write German into the repo. |
...
| **End-of-run STATUS update** (which file, what to move where) | The session-lifecycle protocol is invisible to a remote Codex. |
...
## Kickoff prompt for the remote Codex
...
2. Push the current branch (spec + plan commits) so the remote Codex can see it:
   `git push` (or push the feature branch).
3. Open the remote session (Codex.ai / cloud agent) **on this repo** and paste
   the kickoff prompt.
...
- **Leaving rules in AGENTS.md instead of the plan.** The remote Codex may not
  load AGENTS.md, and even if it does, the plan is what it executes. Inline the
  contract.
...
- **Forgetting the do-not-push instruction.** A capable remote Codex will
